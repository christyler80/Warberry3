...
class Hostname:
    ...
    def findHostnames(self, int_ip, CIDR):
        ...
        try:
            ...
            subprocess.call('crackmapexec -t 50 --timeout 2 smb %s  > Results/hostnames' % CIDR, shell=True)
            #Discover live hosts
            nm=nmap.PortScanner()
            nm_arp=nm.scan(hosts=CIDR,arguments="-sn")
            for x in list(nm_arp.items())[1][1]:
                self.live_ips.append(x)
            
            self.live_ips.remove(int_ip)

            if sys.version_info.major >= 3:
                sorted(self.live_ips, key = ipaddress.IPv4Address)
            else:
                self.live_ips.sort(key=lambda s: map(int, s.split('.')))
